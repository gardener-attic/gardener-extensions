...
func NewEnsurer(etcdBackup *config.ETCDBackup, imageVector imagevector.ImageVector, logger logr.Logger) genericmutator.Ensurer {
	return &ensurer{
		...
		logger:      logger.WithName("vsphere-controlplanebackup-ensurer"),
	}
}
...
func (e *ensurer) ensureChecksumAnnotations(ctx context.Context, template *corev1.PodTemplateSpec, namespace, name string, backupConfigured bool) error {
	if name == v1alpha1constants.StatefulSetNameETCDMain && backupConfigured {
		return controlplane.EnsureSecretChecksumAnnotation(ctx, template, e.client, namespace, vsphere.BackupSecretName)
	}
	return nil
}
...
func (e *ensurer) ensureBackupRestoreContainer(existingContainer *corev1.Container, name string, cluster *extensionscontroller.Cluster) (*corev1.Container, error) {
	...
	image, err := e.imageVector.FindImage(vsphere.ETCDBackupRestoreImageName, imagevector.TargetVersion(cluster.Shoot.Spec.Kubernetes.Version))
	if err != nil {
		return nil, errors.Wrapf(err, "could not find image %s", vsphere.ETCDBackupRestoreImageName)
	}
	...
	if name == v1alpha1constants.StatefulSetNameETCDMain {
		if cluster.Seed.Spec.Backup == nil {
			...
			// TODO martin using OpenStack Swift temporarily
			provider = vsphere.OpenstackStorageProviderName
			env = []corev1.EnvVar{
				{
