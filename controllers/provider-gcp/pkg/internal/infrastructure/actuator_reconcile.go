...
func (a *Actuator) Reconcile(ctx context.Context, infra *extensionsv1alpha1.Infrastructure, cluster *controller.Cluster) error {
	...
	terraformFiles, err := RenderTerraformerChart(a.ChartRenderer, infra, serviceAccount, config, cluster)
	if err != nil {
		return err
	}
	...
	err = tf.
		InitializeWith(TerraformerDefaultInitializer(a.Client, terraformFiles.Main, terraformFiles.Variables, terraformFiles.TFVars)).
		Apply()
	if err != nil {
