...
func Add(mgr manager.Manager, args AddArgs) error {
	args.ControllerOptions.Reconciler = extensionscontroller.OperationAnnotationWrapper(
		...
		NewReconciler(mgr, args.Actuator),
	)
...
func add(mgr manager.Manager, args AddArgs) error {
	...
	if err := ctrl.Watch(&source.Kind{Type: &extensionsv1alpha1.Cluster{}}, &extensionshandler.EnqueueRequestsFromMapFunc{
		ToRequests: extensionshandler.SimpleMapper(ClusterToInfrastructureMapper(mgr.GetClient(), args.Predicates), extensionshandler.UpdateWithNew),
	}); err != nil {
		return err
	}
